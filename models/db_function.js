...
module.exports = {
	...
	create_post: function(user_id, post_type, post_date, way_of_travelling, travel_start_date, travel_end_date, from_city, to_city, description, title, travel_type){
		pg.connect(process.env.DATABASE_URL, function(err, client, done) {	
			var query_string = "INSERT INTO product_post VALUE (default, "+user_id+", \'"+post_type+"\', \'"+ post_date + "\', \'"+way_of_travelling+"\', \'"+travel_start_date+"\', \'"+travel_end_date+"\', "+from_city+", "+to_city+", \'"+description+"\', \'"+title+"\', \'"+travel_type+"\', null) RETURNING post_id";
			console.log(query_string);
			client.query(query_string, function(err, result){
				done();
				if (err) throw err;
